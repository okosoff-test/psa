...
// Hockey Skating School module (isolated from weekly pickup-hockey roster state)
const { initSchoolDatabase } = require('./school-module')({ app, pool, isAuthorizedAdminRequest });
...
function getRosterReleaseAnnouncementText() {
    const rosterMessage = String(rosterReleaseAnnouncementText || '').trim();
    if (rosterMessage) return rosterMessage;
    return buildRosterReleasePaymentAnnouncement();
}
...
async function initDatabase() {
    ...
    try {
        ...
        // Keep skating-school schema independent from the weekly pickup-hockey tables.
        await initSchoolDatabase();
...
function getNextAutoPromoteWaitlistIndex(options = {}) {
    const preferGoalie = options && typeof options.preferGoalie === 'boolean' ? options.preferGoalie : null;
    return waitlist.findIndex(p => {
        if (!p || p.bypassAutoPromote === true) return false;
        if (preferGoalie === null) return true;
        return !!p.isGoalie === preferGoalie;
    });
}

function extractWaitlistPlayerToPromote(options = {}) {
    const index = getNextAutoPromoteWaitlistIndex(options);
...
function getTargetSkaterCounts(totalSkaters) {
    const total = Math.max(0, Number(totalSkaters) || 0);
    return {
        white: Math.ceil(total / 2),
        dark: Math.floor(total / 2)
    };
}

function countSkatersOnTeam(team = []) {
...
function generateFairTeams() {
    ...
    const assignSkatersAndOptimize = (initialWhite = [], initialDark = [], label = 'default') => {
        ...
        const targetSkaters = getTargetSkaterCounts(skaters.length);
        ...
        for (let start = 0; start < skaters.length; start += tierSize) {
            ...
            tier.forEach((skater, index) => {
                ...
                const canAddWhite = countSkatersOnTeam(whiteTeam) < targetSkaters.white;
                const canAddDark = countSkatersOnTeam(darkTeam) < targetSkaters.dark;
                const tryWhite = canAddWhite ? [...whiteTeam, { ...skater, team: 'White' }] : null;
                const tryDark = canAddDark ? [...darkTeam, { ...skater, team: 'Dark' }] : null;
                const whiteObjective = tryWhite ? computeTeamBalanceObjective(tryWhite, darkTeam) + (preferredTeam === 'White' ? -0.08 : 0) : Number.POSITIVE_INFINITY;
                const darkObjective = tryDark ? computeTeamBalanceObjective(whiteTeam, tryDark) + (preferredTeam === 'Dark' ? -0.08 : 0) : Number.POSITIVE_INFINITY;
...
app.post('/api/admin/add-player', async (req, res) => {
    const { password, sessionToken, firstName, lastName, nickname, phone, paymentMethod, piaDate, rating, isGoalie, toWaitlist, assignTeam } = req.body;
    if (!isAuthorizedAdminRequest(req)) return res.status(401).send("Unauthorized");
    ...
    const newPlayerPaymentStatus = isGoalieBool ? 'paid' : (String(normalizedPaymentMethod).toLowerCase() === 'pia' ? 'pia' : 'owes');
    const cleanPiaDate = newPlayerPaymentStatus === 'pia' ? normalizePiaDate(piaDate) : '';
    const requestedTeam = assignTeam === 'White' || assignTeam === 'Dark' ? assignTeam : null;
